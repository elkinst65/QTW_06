...
This is an example of the training input:
...
`Table 1 - Data Element Description for the training data`
...
![Figure 1 - An example of k-Nearest Neighbors from scikit-learn [[2]](#REF02)](knnExample.png) 


In part 1 of the case study, the author's original decision to reject an AP is revisited to see if this was the best decision. In part 2, the prediction of X,Y based on the average of it's neighbors is revisited and modified to a weighted sum of X,Y to determine if this change will produce better predictions. Alternate weights will also be analyzed.
...
#  Figure 2 - Number of reports at each location for the rejected and chosen APs.
```

The maps show there were near equal amounts of training points captured for both APs. A visual comparison of these two diagrams does not give us sufficient insight into which MAC address should be kept and which should be rejected. Now, let us examine the distribution strength of the APs.

[&uarr;TOC](#TOC)


### 1.2.1 <a id='SigStren'> Signal Strength Distributions</a>
Plot the signal strength distributions for each AP per angle for better understanding at location (X=2, Y=12). A more thorough analysis would examine the distributions at all locations points, but time did not permit this in-depth analysis. Instead, we will later create a summary statistic to analyze.

```{r}
df = subset(offline, posX ==2 & posY == 12 & !(mac %in% remainingAP), c(signal, angle, mac))
plotBoxplotSignalStrength(df)
# Figure 3 - Signal strength distribution across both Access Points
```

The higher the decibel-milliwatts (dBm) means a stronger signal strength which indicates a closer physical position to the AP since signal strength decreases exponentially with distance. The distribution shows AP "e1:c0" has a stronger signal strength at this location than "dd:cd." But, the two of them together cover a wider range of signal coverage. What would this plot look like if we moved the fixed location point on the other side of the building at (X=33, Y=3)?

Next, we plot the signal strength distributions for each AP per angle for better understanding at location (X=33, Y=3). 
 
```{r}
# Let's focus on the fixed point (33,3)
df = subset(offline, posX == 33 & posY == 3 & !(mac %in% remainingAP), c(signal, angle, mac))
plotBoxplotSignalStrength(df)
# Figure 4 - Signal strength distribution for a point at the other end of the building
```
This plot is similar to the previous distribution but with one outlier. At this point we know signal strength is different for the two access points. We do not know if this will affect our prediction accuracy. Now, let us analyze the two AP's signal strength distribution over angles.
...
# Figure 5 - Signal strength distribution across angles
```
The above plot shows the rejectedAP "dd:cd" has a higher signal density at lower strength while the acceptedAP "e1:c0" has a higher density coverage at a higher strength. The distributions show many of the plots are normally distributed while some of them, (e.g., "dd:cd" at 180 and "e1:c0" at 315) are heavily skewed or even bimodal. Looking at each AP from top to bottom, there seems to exist an obvious pattern with respect to angle. This would suggest the median of each distribution varies by angle which can be attributed to a difference in perceived strength. This means the angle of the device contributes to the APs strength of signal.

#### Average Signal Strength Distributions
Plot the signal strength distribution for all locations through the average signal strength for all locations by angle by creating a summary statistic for all possible locations.
...
# Figure 6 - Standard deviation of average signal strength
```

This above figure shows there is considerable overlap between the two distributions of signal strength. This suggest there may be value in keeping both APs in the k-NN model.
...
tabonlineXYA
# Table 4 - Test data counts per location and angle
```
The above output shows that each angle has test data meaning we can test all angles for accuracy.

Let's Reshape the test data where each AP is in a column. 
```{r}
onlineSummary = castOnline(online)
#confirm rejected AP is included
dim(onlineSummary)
names(onlineSummary)
# Table 5 - Reshaped test data set
```
...
# Table 6 - Accuracies for the various scenarios
```
The above results show that including both APs yielded a better prediction accuracy followed by keeping chosenAP and then rejectedAP. But was the difference significant or just do to chance? To answer this question, a null hypothesis should be performed.

Our null hypothesis is that all scenarios yield the same error. Our alternative hypothesis is that at least one scenario yields a different error from the others. 

* Ho: None Error = ChosenAP Error = RejectedAP Error
* Ha: At least one scenario is different.

The next step is test our hypothesis with an ANOVA test. This step is left for future work. Therefore, we cannot reject Ho at this time.

We will proceed with our analysis with both APs and perform a cross-validation to determine the best k. Three angles were used. Future work would include parameterization of the angles from 1 to 3 to determine the best m. 
...
plotSSErrors(err, K)
# Figure 7 - Estimated ideal 'k'
```
Given the plot above, the best k shows 9, but this changes if you rerun the cross-validation. Experimentation with k yielded 5 with the lowest error of 210. Thus, it was used below.
...
Our analysis of three scenarios that included both ChosenAP and RejectedAP and each exclused showed that there might be a potential gain from including both APs. Indeed the accuracy results showed including both APs yielded lower errors. However, this difference in errors was not analyzed for significance and was left for future work. A cross-validation for both APs scenario was performed to find optimal k. Analysis for optimal angle, m, was not peformed and left for future work.
...
![Figure 8 - Contour plot of power for measurements against AP '97:8D' at 0 degrees orientation](contour1.png)

![Figure 9 - Contour plot of power for measurements against AP 'E1:C0' at 0 degrees orientation](contour2.png)
...
![Figure 10 - Red lines indicate distance error between predicted location (*) and true location (black dots) using k=4](errors.png)
...
*Table 7 - Comparison of predicted location errors between power-based and signal-strength-based approaches*
...
![*Figure 11 - Signal strength from AP @ 1,14*](SignalStrength1.png)
...
*Table 8 - Nearest neighbors for the target's signal strength*
...
![*Figure 12 - Signal strength from AP @ 33.5,2.8*](SignalStrength2.png)
...
*Table 9 - Nearest neighbors for the target's signal strength across two APs*
...
*Table 10 - Nearest neighbors for the target's strongest and weakest signal strength*
...
In this exercise, we investigated using an alternative k-nearest neighbors prediction method using the weighted 
K-nearest neighbors approach.  The calculation of the Euclidean distance error was found to be 1194, whereas the Mean SS error was found to be 210.  The Euclidean distance weighted k-nearest neighbors method had the highest error, while the Mean SS weighted K-nearest neighbors method had the smaller error.

| k |Euclidean distance error|Mean SS error|
|:-:|:-----------------:|:-----------------:|
|5 |1194|210|
...
## <a id="Future">Future Study</a>
To take our work further, something to consider for a future endeavour would be to analyze the results of the K-Nearest Neighbors analysis for each Chosen and Rejected Access Point by the use of an F-test. The results from this study would help to support our findings in this case study by accepting or rejecting the null hypothesis ($H_O$) of no statistical difference between the results. Once these results are discovered, an analysis of variance could be conducted as well to put the nail in the coffin. 


***
# <a id='References'>References</a>
* <a id='REF01'></a> [1] Nolan, Deborah; Data Science in R; 2015; Predicting Location via Indoor Positioning Systems; Chapter 1. 

* <a id='REF02'></a> [2] Scikit-learn k-NN; http://scikit-learn.org/stable/modules/neighbors.html#nearest-neighbor-algorithms. 
